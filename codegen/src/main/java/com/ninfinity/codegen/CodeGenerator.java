...
public class CodeGenerator {
	...
	public static void Generate(String entityName, String sourcePath, String destPath,String type, String modName) {
		...
		EntityDetails entityDetails = getEntityDetails(sourcePath,entityName);
		...
		if (type.equalsIgnoreCase("other")) {
			...
			generateFiles(uiTemplate, root, destFolder);
			generateBackendFiles(root, destPath);
			generateRelationDto(relationMap,root,destPath,entityName);
		}
	...
	private static EntityDetails getEntityDetails(String entityPath, String entityName) {
		...
		try {
			...
			for (Field field : fields)
			{
				...
				{
					System.out.println(" -- asd : " + a.annotationType().toString());
	...
	private static List<Map<String,String>> getFieldsIfJoinTable(String EntityPackage , String EntityName)
	{
		...
		try {
			...
			for (Field field : fields)
			{
				...
				for(Annotation a : annotations)
				{
					System.out.println(" -- as : " + a.annotationType().toString());
					if(a.annotationType().toString().equals("interface javax.persistence.ManyToOne"))
					{
						String str = field.getType().toString();
						int index = str.lastIndexOf(".")+1;
				...
				if(relationFields.get("fieldType") != null)
				{
					...
					String referenceColumn=findPrimaryKey(entity.concat("." + relationFields.get("fieldType")),relationFields.get("fieldType"));
	...
	private static Map<String,FieldDetails> getFields(String entityPackage, String entityName)
	{
		Map<String,FieldDetails> fieldsMap = new HashMap<>();
