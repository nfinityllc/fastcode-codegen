...
flowableAdminApp.controller('ProcessDefinitionController', ['$scope', '$rootScope', '$http', '$timeout','$location','$routeParams', '$modal', '$translate', '$q', 'gridConstants',
    ...
    $scope.loadProcessInstances = function() {
      ...
      $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/process-definitions/' + $scope.definition.id +'/process-instances'}).
      success(function(data, status, headers, config) {
        ...
        $scope.loadJobs = function() {
            ...
            $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/process-definitions/' + $scope.definition.id +'/jobs'}).
            success(function(data, status, headers, config) {
                $scope.jobs = data;
        ...
        $scope.loadDecisionTables = function() {
            ...
            $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/process-definition-decision-tables/' + $scope.definition.id}).
            success(function(data, status, headers, config) {
        ...
        $scope.loadFormDefinitions = function() {
            // Load forms
            $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/process-definition-form-definitions/' + $scope.definition.id}).
            success(function(data, status, headers, config) {
		...
		$scope.executeWhenReady(function() {
		    // Load definition
		    $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/process-definitions/' + $routeParams.definitionId}).
		    success(function(data, status, headers, config) {
		        $scope.definition = data;
...
flowableAdminApp.controller('EditProcessDefinitionCategoryModalCtrl',
  ...
  $scope.ok = function () {
    ...
    $http({method: 'PUT', url: FlowableAdmin.Config.baseUrl +  '/app/rest/admin/process-definitions/' + $scope.model.id, data: data}).
      success(function(data, status, headers, config) {
