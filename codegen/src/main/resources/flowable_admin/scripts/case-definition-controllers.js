...
flowableAdminApp.controller('CaseDefinitionController', ['$scope', '$rootScope', '$http', '$timeout','$location','$routeParams', '$modal', '$translate', '$q', 'gridConstants',
    ...
    $scope.loadCaseInstances = function() {
      ...
      $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/case-definitions/' + $scope.definition.id +'/case-instances'}).
      success(function(data, status, headers, config) {
        ...
        $scope.loadJobs = function() {
            ...
            $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/case-definitions/' + $scope.definition.id +'/jobs'}).
            success(function(data, status, headers, config) {
                $scope.jobs = data;
        ...
        $scope.loadDecisionTables = function() {
            ...
            $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/case-definition-decision-tables/' + $scope.definition.id}).
            success(function(data, status, headers, config) {
        ...
        $scope.loadFormDefinitions = function() {
            // Load forms
            $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/case-definition-form-definitions/' + $scope.definition.id}).
            success(function(data, status, headers, config) {
		...
		$scope.executeWhenReady(function() {
		    // Load definition
		    $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/case-definitions/' + $routeParams.definitionId}).
		    success(function(data, status, headers, config) {
		        $scope.definition = data;
...
flowableAdminApp.controller('EditCaseDefinitionCategoryModalCtrl',
  ...
  $scope.ok = function () {
    ...
    $http({method: 'PUT', url: FlowableAdmin.Config.baseUrl +  '/app/rest/admin/case-definitions/' + $scope.model.id, data: data}).
      success(function(data, status, headers, config) {
