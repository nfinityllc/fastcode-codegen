...
flowableAdminApp.controller('CmmnTaskController', ['$scope', '$rootScope', '$http', '$timeout','$location','$routeParams', '$modal', '$translate', '$q', 'gridConstants',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q, gridConstants) {
		...
		$scope.loadTask = function() {
			...
			// Load task
			$http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/cmmn-tasks/' + $routeParams.taskId}).
   ...
   .then(function(headers) {
		...
		$scope.loadSubTasks = function() {
			...
			$http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/cmmn-tasks/' + $scope.task.id +'/subtasks'}).
			success(function(data, status, headers, config) {
				$scope.subTasks = data;
		...
		$scope.loadVariables = function() {
			...
			$http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/cmmn-tasks/' + $scope.task.id +'/variables'}).
			success(function(data, status, headers, config) {
		...
		$scope.loadIdentityLinks = function() {
			...
			$http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/cmmn-tasks/' + $scope.task.id +'/identitylinks'}).
			success(function(data, status, headers, config) {
...
flowableAdminApp.controller('CmmnCompleteModalInstanceCtrl',
  ...
  $scope.ok = function () {
	  ...
	  $http({method: 'POST', url: FlowableAdmin.Config.baseUrl +  '/app/rest/admin/cmmn-tasks/' + $scope.task.id, data: {action: 'complete'}}).
  	  success(function(data, status, headers, config) {
...
flowableAdminApp.controller('CmmnResolveTaskModalInstanceCtrl',
  ...
  $scope.ok = function () {
	  $scope.status.loading = true;
	  $http({method: 'POST', url: FlowableAdmin.Config.baseUrl +  '/app/rest/admin/cmmn-tasks/' + $scope.task.id, data: {action: 'resolve'}}).
  	  success(function(data, status, headers, config) {
  		$modalInstance.close(true);
  		$scope.status.loading = false;
      }).error(function(data, status, headers, config) {
...
flowableAdminApp.controller('CmmnDeleteTaskModalInstanceCtrl',
  ...
  $scope.ok = function () {
	  $scope.status.loading = true;
	  $http({method: 'DELETE', url: FlowableAdmin.Config.baseUrl +  '/app/rest/admin/cmmn-tasks/' + $scope.task.id}).
    	success(function(data, status, headers, config) {
    		$modalInstance.close(true);
	  		$scope.status.loading = false;
        }).
...
flowableAdminApp.controller('CmmnDelegateModalInstanceCtrl',
  ...
  $scope.ok = function () {
	  $scope.status.loading = true;
	  $http({method: 'POST', url: FlowableAdmin.Config.baseUrl +  '/app/rest/admin/cmmn-tasks/' + $scope.task.id, data: {action: 'delegate', assignee: $scope.status.user}}).
  	  success(function(data, status, headers, config) {
...
flowableAdminApp.controller('CmmnAssignModalInstanceCtrl',
    ['$rootScope', '$scope', '$modalInstance', '$http', 'task', function ($rootScope, $scope, $modalInstance, $http, task) {
	  ...
	  $scope.ok = function () {
		  ...
		  $http({method: 'PUT', url: FlowableAdmin.Config.baseUrl +  '/app/rest/admin/cmmn-tasks/' + $scope.task.id, data: {assignee: rawUserValue}}).
	  	  success(function(data, status, headers, config) {
...
flowableAdminApp.controller('CmmnEditTaskModalInstanceCtrl',
    ['$rootScope', '$scope', '$modalInstance', '$http', 'task', function ($rootScope, $scope, $modalInstance, $http, task) {
	  ...
	  $scope.ok = function () {
		  $scope.status.loading = true;
		  $http({method: 'PUT', url: FlowableAdmin.Config.baseUrl +  '/app/rest/admin/cmmn-tasks/' + $scope.task.id, data: $scope.model}).
	  	  success(function(data, status, headers, config) {
	  		$modalInstance.close(true);
	  		$scope.status.loading = false;
	      }).error(function(data, status, headers, config) {
