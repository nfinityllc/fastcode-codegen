...
flowableAdminApp.controller('EventSubscriptionController', ['$scope', '$rootScope', '$http', '$timeout','$location','$routeParams', '$modal', '$translate',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate) {
		...
		$scope.triggerEvent = function() {
			...
			$http({method: 'POST', 
				url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/event-subscriptions/' + $scope.eventSubscription.id,
				data: dataForPost}).
		...
		$scope.loadEventSubscription = function() {
			...
			$http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/event-subscriptions/' + $routeParams.eventSubscriptionId}).
	    	success(function(data, status, headers, config) {
