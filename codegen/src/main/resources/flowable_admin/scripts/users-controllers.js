...
flowableAdminApp.controller('UsersController', ['$scope', '$rootScope', '$http', '$timeout','$location', '$modal', '$translate', '$q',
    function ($scope, $rootScope, $http, $timeout, $location, $modal, $translate, $q) {
        ...
        $scope.loadUsers = function() {
        	$http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/users'}).
...
flowableAdminApp.controller('DeleteUserModalInstanceCtrl',
  ...
  $scope.ok = function () {
	  ...
	  $http({method: 'DELETE', url: FlowableAdmin.Config.baseUrl +  '/app/rest/users/' + $scope.user.login}).
    	success(function(data, status, headers, config) {
...
flowableAdminApp.controller('EditUserModalInstanceCtrl',
  ...
  $scope.ok = function () {
	  $scope.status.loading = true;
	  $http({method: 'PUT', url: FlowableAdmin.Config.baseUrl +  '/app/rest/users/' + $scope.user.login, data: $scope.model}).
  	  success(function(data, status, headers, config) {
  		  $modalInstance.close(true);
  		  $scope.status.loading = false;
      }).error(function(data, status, headers, config) {
...
flowableAdminApp.controller('ChangePasswordModalInstanceCtrl',
    ['$scope', '$modalInstance', '$http', 'user', function ($scope, $modalInstance, $http, user) {
	  ...
	  $scope.ok = function () {
		  $scope.status.loading = true;
		  $http({method: 'PUT', url: FlowableAdmin.Config.baseUrl +  '/app/rest/users/' + $scope.user.login + '/change-password', data: $scope.model}).
	  	  success(function(data, status, headers, config) {
	  		  $modalInstance.close(true);
	  		  $scope.status.loading = false;
	      }).error(function(data, status, headers, config) {
...
flowableAdminApp.controller('NewUserModalInstanceCtrl',
  ...
  $scope.ok = function () {
	  $scope.status.loading = true;
	  $http({method: 'POST', url: FlowableAdmin.Config.baseUrl +  '/app/rest/users', data: $scope.model, ignoreErrors: true}).
  	  success(function(data, status, headers, config) {
