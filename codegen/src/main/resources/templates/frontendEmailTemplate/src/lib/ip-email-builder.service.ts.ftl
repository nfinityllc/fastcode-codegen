...
export class IpEmailBuilderService implements OnInit {
  ...
  private getHeaders(): HttpHeaders {
    return new HttpHeaders({
      ...
      'X-Api-Key': this._config.xApiKey
    });
  }
    ...
    return this._http
      ...
      .pipe(
        ...
        tap(({ error }) => {
          ...
          if (error) {
            this.notify(
              'It seems to be an error, try to change your email ID.'
            );
          } else {
            this.notify(
              'Test email has been sent. Can be in Junk Folder!',
              'Thanks',
              10000
            );
          }
  ...
  onChanges$(): Observable<[IPDefaultEmail, string]> {
    return combineLatest(
      this.getEmailAsObservalble$(),
      this.getTemplateAsObservale$()
    );
  }
