...
public class UsersCustomRepositoryImpl implements UsersCustomRepository{
	...
	public Page<PermissionsEntity> getAllPermissions(Long usersId, List<SearchFields> search,String operator, Pageable pageable)  {
		...
		Join<PermissionsEntity, UsersEntity> join = fromPermissions.join("users");
		List<Predicate> conditions = new ArrayList();
		if(operator.equalsIgnoreCase("notEqual"))
		conditions.add(criteriaBuilder.notEqual(join.get("id"), usersId));
		else
		conditions.add(criteriaBuilder.equal(join.get("id"), usersId));
