...
import [=PackageName].application.Authorization.[=AuthenticationTable]permission.Dto.Create[=AuthenticationTable]permissionInput;
...
import [=PackageName].application.Authorization.[=AuthenticationTable]permission.Dto.Update[=AuthenticationTable]permissionInput;
...
public class [=AuthenticationTable]permissionControllerTest {
	...
	public [=AuthenticationTable]permissionEntity createEntity() {
		...
		<#if Flowable!false>
			...
			idmIdentityService.createUser([=AuthenticationTable?uncap_first]permission.get[=AuthenticationTable](), actIdUser);
		</#if>
		...
		[=AuthenticationTable?uncap_first]permission.setPermissionId(DEFAULT_PERMISSION_ID );
		<#if (AuthenticationType!="none" && !UserInput??)>
		[=AuthenticationTable?uncap_first]permission.set[=AuthenticationTable]Id(permission.getId());
	...
	public Find[=AuthenticationTable]ByIdOutput create[=AuthenticationTable]ByIdOuput()
	{
	    ...
	    [=AuthenticationTable?uncap_first].setEmailAddress("u1@g.com");
	    [=AuthenticationTable?uncap_first].setFirstName("U");
	    [=AuthenticationTable?uncap_first].setLastName("2");
		<#else>
  		<#list Fields as key,value>
	...
	public [=AuthenticationTable]permissionEntity createNewEntityForDelete() {
		...
		PermissionEntity permission = createPermissionEntity();
		permission.setName("P4");
		permission.setId(2L);
		permission=permissionRepository.save(permission);
		
		[=AuthenticationTable]permissionEntity [=AuthenticationTable?uncap_first]permission = new [=AuthenticationTable]permissionEntity();
		[=AuthenticationTable?uncap_first]permission.setPermissionId(2L);
		<#if (AuthenticationType!="none" && !UserInput??)>
	...
	public [=AuthenticationTable]permissionEntity createNewEntityForUpdate() {
		...
		permission=permissionRepository.save(permission);
		[=AuthenticationTable]permissionEntity [=AuthenticationTable?uncap_first]permission = new [=AuthenticationTable]permissionEntity();
		[=AuthenticationTable?uncap_first]permission.setPermissionId(5L);
		<#if (AuthenticationType!="none" && !UserInput??)>
		[=AuthenticationTable?uncap_first]permission.set[=AuthenticationTable]Id(user.getId());
		<#else>
		<#if PrimaryKeys??>
	...
	public void FindById_IdIsValid_ReturnStatusOk() throws Exception {
		 ...
		 mvc.perform(get("/[=AuthenticationTable?uncap_first]permission/permissionId:3,<#if (AuthenticationType!="none" && !UserInput??)>[=AuthenticationTable?uncap_first]Id:3<#else><#if PrimaryKeys??><#list PrimaryKeys as key,value>[=AuthenticationTable?uncap_first][=key?cap_first]:3<#sep>,</#list></#if></#if>")
	...
	public void FindAll_SearchIsNotNullAndPropertyIsNotValid_ThrowException() throws Exception {
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/[=AuthenticationTable?uncap_first]permission?search=id[equals]=1&limit=10&offset=1")
				 .contentType(MediaType.APPLICATION_JSON))
	...
	public void Get[=AuthenticationTable]_IdIsNotEmptyAndIdIsNotValid_ThrowException() throws Exception {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/[=AuthenticationTable?uncap_first]permission/permissionId:3/[=AuthenticationTable?uncap_first]")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void GetPermission_IdIsNotEmptyAndIdIsNotValid_ThrowException() throws Exception {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/[=AuthenticationTable?uncap_first]permission/permissionId:3/permission")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk())).hasCause(new EntityNotFoundException("Invalid id=permissionId:3"));
