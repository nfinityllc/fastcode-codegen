...
public class GetUserInput {
	...
	public static List<String> getRelationInput(List<String> classList, List<String> relationClassList, String source,
				...
				for (Map.Entry<String, RelationDetails> entry : relationDetails.entrySet()) {
					for (RelationDetails e : relationList) {
						if (e.geteName().equals(str) && e.getRelation().equals(entry.getValue().getRelation())) {
							System.out.println("\nFor entities " + str + "-" + entry.getValue().geteName() + " having "
									+ entry.getKey() + " relationship , which one is the owner entity ? ");
	...
	public static List<RelationDetails> getRelationList(List<String> classList, List<String> relationClassList,
				...
				for (Map.Entry<String, RelationDetails> entry : relationInput.entrySet()) {
					if (entry.getKey().equals("ManyToMany") || entry.getKey().equals("OneToOne")) {
						...
						for (int i = 0; i < relationList.size(); i++) {
							if (relationList.get(i).geteName().contains(str)
