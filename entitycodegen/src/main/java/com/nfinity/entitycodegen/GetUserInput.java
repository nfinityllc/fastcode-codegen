...
public class GetUserInput {
	...
	public static List<String> getRelationInput(List<Class<?>> classList, List<String> relationClassList, String source,String packageName, Scanner scanner)
	{
			...
			if(!relationClassList.contains(entityName))
			{
				...
				for (Map.Entry<String, RelationDetails> entry : relationDetails.entrySet()) {
					for(RelationDetails e : relationList)
					{
						...
						{
							System.out.println("\nFor entities " + className + "-" + entry.getValue().geteName() + " having " + entry.getKey()
			...
			if(!relationClassList.contains(entityName))
			{
				...
				for (Map.Entry<String, RelationDetails> entry : relationInput.entrySet()) {
					if(entry.getKey().equals("ManyToMany"))
					{
						...
						{
							if(relationList.get(i).geteName().contains(className) && relationList.get(i).getRelation().equals(entry.getKey()))
							{
