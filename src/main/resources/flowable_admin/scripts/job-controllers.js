...
flowableAdminApp.controller('JobController', ['$scope', '$rootScope', '$http', '$timeout','$location','$routeParams', '$modal', '$translate',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate) {
		...
		$scope.executeJob = function() {
			$http({method: 'POST', url: FlowableAdmin.Config.baseUrl +  '/app/rest/admin/jobs/' + $scope.job.id}).
		...
		$scope.moveJob = function() {
			$http({method: 'POST', url: FlowableAdmin.Config.baseUrl +  '/app/rest/admin/move-jobs/' + $scope.job.id + '?jobType=' + $scope.jobType.param}).
        	success(function(data, status, headers, config) {
		...
		$scope.loadJob = function() {
			$scope.job = {};
			$http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/jobs/' + $routeParams.jobId + '?jobType=' + $scope.jobType.param}).
	    	success(function(data, status, headers, config) {
	            $scope.job = data;

	            if ($scope.job.exceptionMessage) {
	            	// Fetch the full stacktrace, associated with this job
	            	$http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/jobs/' + $scope.job.id + '/stacktrace?jobType=' + $scope.jobType.param}).
	            	success(function(data, status, headers, config) {
...
flowableAdminApp.controller('DeleteJobModalInstanceCtrl',
    ['$rootScope', '$scope', '$modalInstance', '$http', 'job', 'jobType', function ($rootScope, $scope, $modalInstance, $http, job, jobType) {
	  ...
	  $scope.ok = function () {
		  ...
		  $http({method: 'DELETE', url: FlowableAdmin.Config.baseUrl +  '/app/rest/admin/jobs/' + $scope.job.id + '?jobType=' + jobType.param}).
	    	success(function(data, status, headers, config) {
