...
flowableAdminApp.controller('FormDeploymentController', ['$scope', '$rootScope', '$http', '$timeout','$location','$routeParams', '$modal', '$translate', '$q',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q) {
		...
		$scope.returnToList = function() {
			$location.path("/flowable-admin/form-deployments");
		};
		
		$scope.openDefinition = function(definition) {
			if (definition && definition.getProperty('id')) {
				$location.path("/flowable-admin/form-definition/" + definition.getProperty('id'));
			}
		};
		...
		$scope.deleteDeployment = function() {
            ...
            modalInstance.result.then(function (result) {
                if (result === true) {
                    $http({method: 'DELETE', url: FlowableAdmin.Config.baseUrl +  '/app/rest/admin/form-deployments/' + $routeParams.deploymentId}).
		...
		$scope.executeWhenReady(function() {
		    // Load deployment
		    $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/form-deployments/' + $routeParams.formDeploymentId}).
  	    	    success(function(data, status, headers, config) {
		    ...
		    $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/form-definitions?deploymentId=' + $routeParams.formDeploymentId}).
  	    	    success(function(data, status, headers, config) {
