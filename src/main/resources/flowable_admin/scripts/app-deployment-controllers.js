    ...
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q) {
		...
		$scope.deleteDeployment = function() {
            ...
            modalInstance.result.then(function (result) {
                if (result === true) {
                    $http({method: 'DELETE', url: FlowableAdmin.Config.baseUrl +  '/app/rest/admin/app-deployments/' + $routeParams.deploymentId}).
		...
		$scope.executeWhenReady(function() {
		    // Load deployment
		    $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/app-deployments/' + $routeParams.deploymentId}).
  	    	    success(function(data, status, headers, config) {
		    ...
		    $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/app-definitions?deploymentId=' + $routeParams.deploymentId}).
  	    	    success(function(data, status, headers, config) {
