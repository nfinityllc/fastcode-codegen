    ...
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q) {
		...
		$scope.deleteDeployment = function() {
            ...
            modalInstance.result.then(function (result) {
                if (result === true) {
                    $http({method: 'DELETE', url: FlowableAdmin.Config.baseUrl +  '/app/rest/admin/cmmn-deployments/' + $routeParams.deploymentId}).
		...
		$scope.executeWhenReady(function() {
		    // Load deployment
		    $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/cmmn-deployments/' + $routeParams.deploymentId}).
  	    	    success(function(data, status, headers, config) {
		    ...
		    $http({method: 'GET', url: FlowableAdmin.Config.baseUrl + '/app/rest/admin/case-definitions?deploymentId=' + $routeParams.deploymentId}).
  	    	    success(function(data, status, headers, config) {
